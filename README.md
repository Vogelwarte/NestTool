...
```{r data_prep, eval= FALSE, message=FALSE, warning = FALSE, include=T}
...
```{r nesting, message=FALSE, eval= FALSE, include=T}
...
#saveRDS(nest_model, here("nest_model.rds"))   ## optionally save the model for later use
...
```{r data_extraction, message=FALSE, eval= FALSE, include=T}
...
```{r movement_visualisation, message=FALSE, eval= FALSE, include=T}
...
- In the top right panel, the movement data is displayed on an interactive map. The locations are colour-coded by date and, if required, the movement trajectory can also be displayed. The potential nest location (as determined by the function`data_prep`) is also shown on the map. In addition to the topographic map, a satellite image can be selected as a background map in order to better identify forest areas or individual potential nest trees. The measurement tool allows distances and areas to be measured.
...
```{r shiny_graphic, echo=FALSE, out.height='80%', out.width='80%', fig.align="center"}
knitr::include_graphics(here("plots/shiny_app.png"), dpi = 100)
```
...
```{r combine_data, message=FALSE, eval= FALSE, include=T}
...
```{r demo_summary, message=FALSE, eval= FALSE, include=T}
