---
title: "NestTool workflow description"
author: "Ursin Beeli, Steffen Oppel"
output: rmarkdown::html_vignette
description: description of how to use R functions to predict Red Kite nesting from tracking data
vignette: >
  %\VignetteIndexEntry{NestTool}
...
The **NestTool** package is an informal collection of R functions created by the [Swiss Ornithological Institute](https://www.vogelwarte.ch) to extract demographic information from the tracking data of birds. The functions were developed in the framework of a Red Kite project, and tested and validated mainly for Red Kites. This vignette gives a brief overview of the core functions and the typical workflow when trying to predict nesting and success from tracking data. 
...
devtools::install_github("steffenoppel/NestTool", dependencies=TRUE) # development version - add argument 'build_vignettes = FALSE' to speed it up 
```
...
After loading all the required packages and functions of `NestTool` you can read in your data:
...
source(here("R//train_home_range_detection.r"))
source(here("R//predict_ranging.r"))
source(here("R//train_nest_detection.r"))
source(here("R//predict_nesting.r"))
source(here("R//train_nest_success.r"))
source(here("R//predict_success.r"))
source(here("R//move_metric_extraction.r"))
source(here("R//plot_move_metrics.r"))
source(here("R//movement_visualisation.r"))
...
Next, we use the data.frame summary created by `data_prep` to train a model for nesting behaviour using the function `train_nest_detection`. Here we need to carefully subset the data to only use individual seasons where information about a nest is available. We can then use the resulting model to predict for each individual and season whether the bird initiated a nesting attempt in that season using the function `predict_nesting`.
...
## if no model has been trained, the model statement can be omitted and the Red Kite model from Switzerland will be used
pred_nest <- predict_nesting(model = nest_model$model, trackingsummary = nest_data_input$summary) # note that you can predict to all data, including those without information on whether a nest existed or not

```

When training a model with the function `train_nest_detection`, a graph will be produced that indicates the most important predictor variables and the accuracy in predicting the correct outcome for testing data that were not used to train the model (these are automatically split from the input to `train_nest_detection`). The accuracy is the proportion of cases that were predicted correctly, and ranges between 0 (all predictions were false) to 1 (all predictions were correct). Our model for red kites in Switzerland achieves an accuracy of >95% of correct nesting classification, and we see that the total time spent within the nest radius and the distance between day and night roost locations are important to distinguish between nesting and non-nesting birds.
...
Finally, we use the data.frame created by `predict_nesting` to train a model for breeding success using the function `train_nest_success`. This function will automatically filter out only those data for which success is known (either 'yes' or 'no'), so there is no need to manually subset the input data. We can then use the resulting model to predict for each individual and season whether they nested successfully using the function `predict_success`. This function also relies on the output of nest_prob from the function `predict_nesting`, and will automatically assign all individual seasons to 'no success' if the probability of a nesting attempt predicted by the function `predict_nesting` was below the minimum probability of the training data used to create the nest success model. However, the user can manually adjust that threshold using the input parameter `nest_cutoff` in the function `predict_success`.
...
The above steps provide a quick classification of nesting and success for many tracked individuals, but there will be some for which the predicted probability of nesting or success will be marginal. The package NestTool therefore provides a graphical user interface to visually inspect an individuals movements over a season to quickly determine whether it was nesting successfully or not.
...
The final function is `movement_visualisation` which will open a Shiny App that allows the user to visually inspect the tracks and movement metric graphs for each of the individual seasons that fall below the thresholds of uncertainty. We use the data.frames `movementtrack` and `pot_nests` created by `data_prep`, the data.frame created by `predict_success` and the data.frame created by `move_metric_extraction`. Also, `uncertainty_success` must be specified so that only those individuals are considered for which the automated classification has been highly uncertain. This value should be the same as for `uncertainty` in `move_metric_extraction` to ensure that the same individuals for which movement metrics were calculated can be inspected. Furthermore, `uncertainty_nest` must be defined to filter out individuals with a low probability that a nest is present. It makes sense to choose this value the same as the `nest_cutoff` in `predict_success` to filter out the data at the same threshold. Finally, a path must be defined in which a data.frame containing the most important move metrics and the decisions entered by the user is stored.
...
movement_visualisation(trackingdata = nest_data_input$movementtrack,
                       ...
                       uncertainty_nest = succ_model$nest_cutoff,
                       output_path="output/nest_success_output.csv")
```
...
Also consider an alternative package `nestR`, which we originally tried for this work but found less accurate than our approach.
...
Thanks to Patrick Scherler and Thomas Pfeiffer for kindly sharing example data for use in the package. 

### Acknowledgements

This project has received funding from the European Union’s LIFE Programme and...